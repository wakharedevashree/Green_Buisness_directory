...
let generate_cart_item = () => {
    if (basket.length !== 0) {
        shopping_cart.innerHTML = basket.map((x) => {
            let { id, name, quantity, price, img, item } = x; // Include `item` for quantity
            return `
            <div class='cart_item'>
...
/*let total_amount = () => {
    if (basket.length !== 0) {
        // Calculate total amount by parsing price correctly
        let totalAmount = basket.reduce((total, item) => 
            total + (item.item * parseInt(item.price.replace(' ₹', '').replace(',', ''))), 0);
...
let total_amount = () => {
    if (basket.length !== 0) {
        // Calculate total amount directly using integers
        let totalAmount = basket.reduce((total, item) => 
            total + (item.item * item.price), 0);
